...
if($my_password == $correct_password){
	$sql = "UPDATE tbl_students set archived='$_stats' where stud_id='$stud_id'";
	$res = $conn->query($sql);
	$three = 3;
	if($res){
		$sql1 = "UPDATE tbl_students_work set status='$_stats', isReadStud='$three' where stud_uniq_id='$stud_uniq_id'";
		...
		if ($res1) {
            ...
            if ($query_log_run) {
			    ...
			    $userType = "Teacher";
			    $session_audit = $get_name_id. " archived the student account of ".$full_name_and_id;
