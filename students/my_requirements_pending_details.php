...
<!-- Page Heading -->
    ...
    $count = 1;
    $myfullname_ = $_SESSION['student_fullname_session'];
    $sql = "SELECT * from tbl_students_work where stud_name_and_id='$myfullname_' and name_of_teacher='$student_teacher' and work_status='$work_stats' and status='$stats' and submitted='$submitted' and id='$req_id'";
     $query = $conn->query($sql);
     while($row = $query->fetch_assoc()){
    ?>

    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800"><span><a href="my_requirements_pending.php" class="h3 mb-0 text-gray-800"><b>My Requirements</b></a> / To-Do / <b style="color: #0d6efd"><?php echo $row['title'];?></b> </span></h1>
    </div>
    ...
    <div class="card shadow mb-4">
        <!-- <div class="card-header py-3">
            <a href="my_requirements_pending.php" class="btn btn-sm btn-dark shadow-sm" title="Back to List"><i class="fas fa-long-arrow-alt-left"></i> Back To List</a>
        </div> -->
        <div class="card-header py-3" style="background: #595959;">
            <h6 class="m-0 font-weight-bold text-light">
                <h><i class="fas fa-file-alt"></i> Requirement Details</h><br>
            </h6>
        </div>
        <div class="card-body">
            <table class="table table" width="100%" style="color:  #333333;">
            <form action="my_requirements_submit.php" method="POST" enctype="multipart/form-data">
                ...
                <tr id="">
                    <td style="font-size:16px;"> <b>Instructions :</b> <br>
                ...
                <tr id="">
                    <td style="font-size:16px;"> <b>Attach File :</b><br>
                    <?php if($row['sample_file'] != ""){?><b><a href="../admin/upload_docs/<?php echo $row['sample_file'];?>" target="_blank" title="Download File"><i class="fa fa-file"></i><?php echo $row['sample_file'];?></a> </b>
                ...
                <tr id="">
                    <td style="font-size:16px;"> <b>Date of Submission :</b><br>
                    <?php echo $row['date_of_submission'];?></td>
                </tr>
                <tr id="">
                    <td style="font-size:16px;"> <b>Turned In :</b>
                        <br> <?php
                        //detect the due dates
                            ...
                            if($row['date_of_submission'] < $date){
                                ...
                                echo "<span style='font-size: 15px; margin-left: 0px; color: ghostwhite;' class='badge badge-pill badge-success'>ON-GOING</span>";
                            }
                        ?> 
                    </td>
                </tr>
                ...
                <tr id="">
                    <td style="font-size:16px;"><b>Attach Your File :</b>
                    <div class="form-group">
                        <input type="file" class="form-control" accept="application/pdf"  id="my_file" name="my_file" required="">
                    </div></td>
                </tr>
                <tr id="">
                    <td style="font-size:16px;"> <b>Comment (Optional) : </b><br>
                    <textarea rows="5" style="background: white;" name="student_comment" class="form-control" placeholder="Write Your Comment Here (Optional) "  id="student_comment" aria-describedby=""></textarea></td>
                </tr>
                <tr id="">
                    <?php 
                    session_start();
                    $sql1 = "SELECT Concat(date_,' at ',time_) AS dateTTime1 FROM tbl_students_work where id='$req_id'";
                    $query1 = $conn->query($sql1);
                    while($row1 = $query1->fetch_assoc()){
                        $_SESSION['datetime2'] = $row1['dateTTime1'];
                    }
                ?>
                    <td style="font-size:16px;"><i style="color: #1a8cff;" class="fas fa-user"></i> <?php echo $row['name_of_teacher'];?> • <i style="color: #1a8cff;" class="fas fa-clock"></i> <?php $date_time_posted = $_SESSION['datetime2']; echo $date_time_posted; ?> </td>
                </tr>
                <tr>
                    <td><div><button type="submit" name="submit"  class="btn btn-primary">Submit Your Work <i class="fas fa-check-square"></i></button></td>
                </tr>
            </form>
            </table>
        <?php } ?>
