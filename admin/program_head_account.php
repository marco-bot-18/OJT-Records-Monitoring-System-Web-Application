...
if(isset($_POST['save'])) {
    ...
    if($count > 0){
        ...
        $sql = "INSERT INTO tbl_coordinators(coordinator_id, fname, mname, lname, contact, email, gender, address, password, status, image, academic_yr_sem, civil_stats, userType, program_head_of, uniq_id, isArchived) VALUES 
        ('$coordinator_id', '$fname', '$mname', '$lname', '$contact', '$email', '$gender', '$address', '$password', '$status', '$img', '$academic_yr_sem', '$civil_stats', '$usertype', '$program_head_of', '$uniq_id', '$isArchived')";
        ...
        if($query_run) {
                try {
                    ...
                    $mail->Subject = "e-OJT aCCeSs: Your Have Now Your Account As a Program Head of ".$course;
                ...
                if ($query_log_run) {
                    $sign_image = "";
                    $owner = $fname." ".$mname." ".$lname." (".$coordinator_id.")";
                    $sql1 = "INSERT INTO tbl_signature(image, owner_signature, uniq_id) VALUES 
                    ('$sign_image', '$owner', '$uniq_id')";

                    $query_run1 = mysqli_query($conn, $sql1);
                    if ($query_run1) {
                        $_SESSION['add_PH_success'] = $fname." ".$mname." ".$lname;
    ...
    $mname = ucwords($_POST['mname']);
    $lname = ucwords($_POST['lname']);
    $program = $_POST['program_head_of'];
    $acad_yr_sem = $_POST['acad_yr_sem'];

    if($coordinator_id == ""){
        $fullname = $fname." ".$mname." ".$lname." (".$current_emp_id.")";

        $sql111 = "UPDATE tbl_signature SET owner_signature='$fullname' WHERE uniq_id='$uniq_id'";
        $query111 = $conn->query($sql111);
        if($query111){
            $sql11 = "UPDATE tbl_coordinators SET fname='$fname', mname='$mname', lname='$lname',program_head_of='$program', academic_yr_sem='$acad_yr_sem' WHERE id='$id'";
                ...
                if($query11){
                    ...
                    $query_log_run = mysqli_query($conn, $sql_log);
                    if ($query_log_run) {
                        $_SESSION['edit_PH_success'] = $fname." ".$mname." ".$lname." (".$current_emp_id.")";
                        header('location: program_head_account.php?=program_head_updated_info');
                    }
                    else{
                        echo $conn->error;
                    }   
            ...
            <li class="nav-item active">
                ...
                <div id="collapseAccounts" class="collapse" aria-labelledby="headingUtilities"
                    ...
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item active" href="program_head_account.php">Program Head</a>
                    </div>
                </div>
            </li>
...
<script>
    function checkAvailabilityProgramHeadId1() {
    $("#loaderIcon").show();
        jQuery.ajax({
            url: "includes/check_availability.php",
            data:'coordinator_id2='+$("#coordinator_id2").val(),
            type: "POST",
            success:function(data){
            $("#coordinator_id-availability2").html(data);
            $("#loaderIcon").hide();
        },
        error:function (){}
        });
      ...
      <div class="modal-dialog modal-xl" role="document" style="color: #333333">
        <div class="modal-content modal-xl">
          <div class="modal-header modal-xl">
            <h5 class="modal-title" style="color: #0d6efd;"><h><i class="fas fa-user-tie"></i> Create Account for Program Head</h></h5>
                ...
                <div class="row">
                    ...
                    <form class="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" style="text-transform: capitalize;" name="fname" class="form-control" id="fname" aria-describedby="" placeholder="ex. Juan" required="">
                        </div>
                        <div class="form-group">
                            <label for="">Middle Name <span style="color: red;">*</span></label><br>
                            <input type="text" style="text-transform: capitalize;" name="mname" class="form-control" id="mname" aria-describedby="" placeholder="ex. Santos" required="">
                        </div>
                        <div class="form-group">
                            <label for="">Last Name: <span style="color: red;">*</span></label><br>
                            <input type="text" style="text-transform: capitalize;" name="lname" class="form-control" id="lname" aria-describedby="" placeholder="ex. Dela Cruz" required="">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="">Assign Program Head of: <span style="color: red;">*</span></label><br>
...
<!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800"><span><b>Accounts</b></span> / Program Head</h1>
    </div>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">
                <h6 class="m-0 font-weight-bold text-primary">
                    ...
                    Create Account for Program Head <i class="fas fa-plus-square"></i>
                </button>
                </h6>
            </h6>
        </div>
        <div class="card-header py-3" style="background: #595959;">
            <h6 class="m-0 font-weight-bold" style="color: ghostwhite;">
                <i class="fas fa-user-tie"></i> User Account of Program Heads
            </h6>
        </div>
            <!-- table -->
            <div class="card-body">
                    ...
                    <table class="table table-hover" id="dataTable"  cellpadding="20"  id="example" width="100%" style="color: #333333">
                      ...
                      <tbody>
                        ...
                        <tr id="tra">
                          ...
                          <td><?php echo $row['program_head_of'];?></td>
                          <td>
                          <?php
                          ...
                          <td>
                            <a href="#view<?php echo $id;?>" data-toggle="modal" data-id="" class="btn btn-info" title="View Coordinator's Account Info"><i class="fas fa-eye"></i></a>
                            <a href="#edit<?php echo $id;?>" data-toggle="modal" data-id="" class="btn btn-dark" title="Edit Program Head's Account Info"><i class="fas fa-edit"></i></a>
                            <?php if ($status == 'not active') {?>
                                <a href='program_head_set_active.php?id=<?php echo $id;?>' title="Set Program Head's Account to Active" class='active-btn'><button class='btn btn-success'><i class="fas fa-check-square"></i></button></a>
                            <?php } else if ($status == 'active') { ?>
                                <a href='program_head_set_ban.php?id=<?php echo $id;?>' title="Set the Program Head's Account to Not Active" class='ban-btn'><button class='btn btn-warning'><i class="fas fa-ban"></i></button></a>
                            <?php } ?>
                          ...
                          <div class="modal-dialog modal-lg" role="document" style="color:  #333333;">
                            <div class="modal-content modal-lg">
                              <div class="modal-header modal-lg">
                                <h5 class="modal-title" style="color: #0d6efd;"><h><i class="fas fa-user-tie"></i> Edit Program Head's Details</h></h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                  <span aria-hidden="true">&times;</span>
                                </button>
                              </div>
                              <div class="modal-header modal-lg">
                                ...
                                <i class="fas fa-graduation-cap"></i> <?php echo $row['program_head_of']?> <br>
                              ...
                              <div class="modal-body modal-lg">
                                <form action="" method="POST" enctype="multipart/form-data">
                                    <!-- <div class="form-group"> -->
                                        ...
                                        <!-- <label for="">Employee ID: <span style="color: red;">*</span></label> <br> -->
                                        <input type="text" name="coordinator_id" class="form-control" id="coordinator_id2" aria-describedby="" onBlur="checkAvailabilityProgramHeadId1()" hidden="" placeholder="ex. 990879">
                                        <!-- <div style="padding-top: 5px;">
                                            <span id="coordinator_id-availability2" style="font-size:12px;"></span>
                                        </div> 
                                    ...
                                    <div class="form-group">
                                        <label for="">Assigned Program Head of: <span style="color: red;">*</span></label><br>
                                        <select class="form-control" name="program_head_of" id="course">
                                            <option value='<?php echo $row['program_head_of'];?>' selected=''>Default Program</option>
                                            <?php
                                            include 'includes/db_connect_pdo.php';
                                            //using pdo format
                          ...
                          <div class="modal-dialog">
                            <div class="modal-content" style="color: #333333;">
                              ...
                              <div class="modal-body">
                                <form action="program_head_account_archive.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal fade bd-example-modal-lg" id="view<?php echo $id;?>" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true" data-backdrop="static" data-keyboard="false">
                          <div class="modal-dialog modal-lg" role="document" style="color:  #333333;">
                            <div class="modal-content modal-lg">
                              <div class="modal-header modal-lg">
                                <h5 class="modal-title" style="color: #0d6efd;"><i class="fas fa-user-tie"></i> View Program Head's Details</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                  <span aria-hidden="true">&times;</span>
                                </button>
                              </div>
                              <div class="modal-body modal-lg">
                                ...
                                <div class="container" style="background: rgba(51, 51, 51, 0.9); color: ghostwhite; padding: 20px;">
                                  ...
                                  <div class="row">
                                   ...
                                   <div class="row">
                                    <div class="col">
                                       <label>Name of Program Head :</label>
                                    </div>
                                    <div class="col">
                                  ...
                                  <div class="row">
                                    <div class="col">
                                      <label>Assigned Program Head of:</label>
                                    </div>
                                  
                                    <div class="col">
                                       <font style="font-weight: bold; "><?php echo $row['program_head_of']; ?></font>
                                    </div>
                                  </div>
                                </div>
                                <br>
                                <div class="container" style="color: #333333;">
                                    <h5>Program Head's Additional Information</h5>
                                    <hr>
                                  <div class="row">
                                    <div class="col">
                                      <label>Email :</label>
...
<!-- alert messages -->
<?php
    if (isset($_SESSION['edit_PH_success'])) {?>
        <script>
            swal.fire({
             icon: 'success',
             type : 'success',
             title : 'Program Head Account was Updated Successfully',
             text : '<?php echo $_SESSION['edit_PH_success']?>'
         })
        </script>
    <?php unset($_SESSION['edit_PH_success']);
    }
?>

<?php
    if (isset($_SESSION['add_PH_success'])) {?>
        <script>
            swal.fire({
             icon: 'success',
             type : 'success',
             title : 'Program Head Account was Created Successfully',
             text : '<?php echo $_SESSION['add_PH_success']?>'
         })
        </script>
    <?php unset($_SESSION['add_PH_success']);
    }
?>
    ...
    if (isset($_SESSION['set_active'])) {?>
        <script>
            swal.fire({
             icon: 'success',
             type : 'success',
             title : 'Program Head Account is Created Successfully',
    ...
    if (isset($_SESSION['archived'])) {?>
        <script>
            swal.fire({
             icon: 'success',
             type : 'success',
             title : 'Program Head Account was Archived Successfully!',
